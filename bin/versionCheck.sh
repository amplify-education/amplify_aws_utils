...
if [ "$IS_PULL_REQUEST" != "false" ]; then
    if git diff --quiet "origin/${TARGET_BRANCH}...HEAD" 'amplify_aws_utils' "test" setup.* ./*.pip; then
        echo "No changes found to main code or dependencies: no version change needed"
        exit 0
    fi

    CURRENT_VERSION=$(git show "origin/${TARGET_BRANCH}:amplify_aws_utils/version.py" | sed -n 's/^__version__ = "\(.*\)"$/\1/p')
    NEW_VERSION=$(sed -n 's/^__version__ = "\(.*\)"$/\1/p' amplify_aws_utils/version.py)
